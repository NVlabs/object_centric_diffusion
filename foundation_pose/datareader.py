...
BOP_DIR = f'/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/BOP'
...
def load_cnos_mask(mask_list=None, json_file=f'/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/github/cnos/sam_pbr_ycbv.json'):
  if mask_list is None:
    mask_list = json.load(open(json_file,'r'))
  res = NestDict()
  for c in mask_list:
    scene_id = c['scene_id']
    id_str = f"{int(c['image_id']):06d}"
    ob_id = int(c['category_id'])
    score = c['score']
    # segmentation = rle_to_mask(c['segmentation']).astype(bool)
    if ob_id not in res[scene_id][id_str]:
      res[scene_id][id_str][ob_id] = {
        'mask': [],
        'score': [],
      }
    res[scene_id][id_str][ob_id]['score'].append(score)
    res[scene_id][id_str][ob_id]['mask'].append(c['segmentation'])
  return res
...
class YcbineoatReader:
  ...
  def get_mask(self,i):
    ...
    #       mask = mask[...,c]
    #       break
    mask_image = Image.open(self.color_files[i].replace('rgb','masks')).convert('P')
  ...
  def get_depth(self,i):
    ...
    depth = cv2.imread(self.color_files[i].replace('rgb','depth'), cv2.IMREAD_ANYDEPTH) / 1000.
    depth = cv2.resize(depth, (self.W,self.H), interpolation=cv2.INTER_NEAREST)
    depth[(depth<MIN_DEPTH) | (depth>=self.zfar)] = 0
    return depth
  ...
  def get_gt_mesh(self):
    ...
    mesh = trimesh.load(f'/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/YCB_Video/models/{ob_name}/textured_simple.obj')
    return mesh
    ...
    for i_k, k in enumerate(self.scene_gt[str(name)]):
      if k['obj_id']==ob_id:
        ...
        if mask is not None:  # When multi-instance exists, use mask to determine which one
          ...
          ##########!DEBUG
          # cv2.imwrite(f'/home/bowen/debug/gt_mask.png', ((gt_mask>0)*255).astype(np.uint8))
          # cv2.imwrite(f'/home/bowen/debug/mask.png', mask.astype(np.uint8)*255)
          # pdb.set_trace()
...
class LinemodOcclusionReader(BopBaseReader):
  def __init__(self,base_dir='/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/LINEMOD-O/lmo_test_all/test/000002', zfar=np.inf):
    ...
    # self.symmetry_tfs = {}
    # for ob_id in self.ob_ids:
    #   self.symmetry_tfs[ob_id] = [np.eye(4)]
    #   if ob_id==10:
    #     self.symmetry_tfs[ob_id].append(euler_matrix(0, 0, np.pi))
    #   self.symmetry_tfs[ob_id] = np.array(self.symmetry_tfs[ob_id])
...
class LinemodReader(LinemodOcclusionReader):
  ...
  def get_reconstructed_mesh(self, ob_id):
    mesh = trimesh.load(os.path.abspath(f'{self.base_dir}/../../../bowen_addon/ref_views/ob_{ob_id:07d}/model/model.obj'))
    return mesh


class YcbVideoReader(BopBaseReader):
  def __init__(self, base_dir=f'/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/YCB_Video/test/000048', zfar=np.inf):
    super().__init__(base_dir, zfar=zfar)
    ...
    names = sorted(os.listdir(f'/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/YCB_Video/models/'))
    self.ob_id_to_names = {}
    self.name_to_ob_id = {}
  ...
  def get_gt_mesh(self, ob_id:int, get_posecnn_version=False):
    if get_posecnn_version:
      mesh = trimesh.load(f'/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/YCB_Video/models/{self.ob_id_to_names[ob_id]}/textured_simple.obj')
      return mesh
    mesh_file = self.get_gt_mesh_file(ob_id)
  ...
  def get_reconstructed_mesh(self, ob_id, num_ref_view=16):
    mesh = trimesh.load(os.path.abspath(f'{self.base_dir}/../../bowen_addon/ref_views_{num_ref_view}/ob_{ob_id:07d}/model/model.obj'))
    return mesh
