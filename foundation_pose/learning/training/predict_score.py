...
@torch.no_grad()
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, xyz_map, normal_map=None, mesh_diameter=None, glctx=None, mesh_tensors=None, dataset:TripletH5Dataset=None, cfg=None):
  H,W = depth.shape[:2]
...
class ScorePredictor:
  def __init__(self, amp=True):
    ...
    code_dir = os.path.dirname(os.path.realpath(__file__))
    ckpt_dir = f'{code_dir}/../../../data/model_weight/foundation_pose/{self.run_name}/{model_name}'
    self.cfg = OmegaConf.load(f'{code_dir}/../../../data/model_weight/foundation_pose/{self.run_name}/config.yml')
  ...
  def predict(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None):
    '''
    @rgb: np array (H,W,3)
    '''
    ...
    pose_data = make_crop_data_batch(self.cfg.input_resize, ob_in_cams, mesh, rgb, depth, K, crop_ratio=self.cfg['crop_ratio'], xyz_map=xyz_map_tensor, glctx=glctx, mesh_tensors=mesh_tensors, dataset=self.dataset, cfg=self.cfg, mesh_diameter=mesh_diameter)
    ...
    if not self.multi_pair:
      ...
      for b in range(0, pose_data.rgbAs.shape[0], bs):
        ...
        # A = pose_data.rgbAs[b:b+bs].cuda()
        # B = pose_data.rgbBs[b:b+bs].cuda()
        if pose_data.normalAs is not None:
