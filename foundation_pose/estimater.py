...
from foundation_pose.Utils import erode_depth
from foundation_pose.datareader import *
import itertools
from foundation_pose.learning.training.predict_score import *
from foundation_pose.learning.training.predict_pose_refine import *
import yaml


class FoundationPose:
  def __init__(self, model_pts, model_normals, symmetry_tfs=None, mesh=None, scorer:ScorePredictor=None, refiner:PoseRefinePredictor=None, cfg=None, glctx=None, debug=0, debug_dir='/home/bowen/debug/novel_pose_debug/'):
    self.gt_pose = None
  ...
  def guess_translation(self, depth, mask, K):
    ...
    valid = mask.astype(bool) & (depth>=MIN_DEPTH)
    if not valid.any():
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    if self.debug>=2:
      ...
      valid = xyz_map[...,2]>=MIN_DEPTH
      pcd = toOpen3dCloud(xyz_map[valid], rgb[valid])
      o3d.io.write_point_cloud(f'{self.debug_dir}/scene_raw.ply',pcd)
      cv2.imwrite(f'{self.debug_dir}/ob_mask.png', (ob_mask*255.0).clip(0,255))

    normal_map = None
    valid = (depth>=MIN_DEPTH) & (ob_mask>0)
    if valid.sum()<4:
    ...
    scores, vis = self.scorer.predict(mesh=self.mesh, rgb=rgb, depth=depth, K=K, ob_in_cams=poses.data.cpu().numpy(), xyz_map=xyz_map, normal_map=normal_map, mesh_tensors=self.mesh_tensors, glctx=self.glctx, mesh_diameter=self.diameter, get_vis=self.debug>=2)
    if vis is not None:
