...
def save_scene_pc(demo, frame_idx, save_dir):
    ...
    grasp_obj_pose = obs.misc["grasp_obj_pose"] if "grasp_obj_pose" in obs.misc else obs.misc["stage0_grasp_obj_pose"]
    target_obj_pose = obs.misc["target_obj_pose"] if "target_obj_pose" in obs.misc else obs.misc["stage0_target_obj_pose"]
    points = np.vstack([
...
def run_all_variations(i, lock, task_index, variation_count, results, file_lock, tasks, data_dict, peract_demo_dir=None):
    ...
    while True:
        ...
        for var in range(possible_variations):
            ...
            task_env._scene.enable_stage = True # !! start tracking multi-stage poses
        ...
        for ex_idx in range(FLAGS.episodes_per_task):
            ...
            while attempts > 0:
                ...
                with file_lock:
                    ...
                    def start_end_discovery(demo):
                        start_end = []
                        gripper_close_at = -1
                        gripper_open_at = -1

                        prev_gripper_open = 1
                        for idx, gripper_open in enumerate([obs.gripper_open for obs in demo]):
                            if gripper_open == 0 and prev_gripper_open == 1: # gripper_close
                                if idx - prev_gripper_open > 10:
                                    gripper_close_at = idx
                            
                            if gripper_open == 1 and prev_gripper_open == 0: # gripper_close
                                if idx - gripper_close_at > 10:
                                    gripper_open_at = idx
                                    start_end.append((gripper_close_at, gripper_open_at))

                            prev_gripper_open = gripper_open
                        return start_end
                    
                    start_end = start_end_discovery(demo)

                    # check if stage number is correct
                    if task_env.get_name() == "place_cups":
                        correct_stage_num = variation+1
                    elif task_env.get_name() == "stack_cups":
                        correct_stage_num = 2
                    elif task_env.get_name() == "stack_blocks":
                        correct_stage_num = task_env._task.blocks_to_stack
                    else:
                        raise NotImplementedError
                    if not len(start_end) == correct_stage_num:
                        print("Skip", len(start_end), correct_stage_num)
                        continue
                    ...
                    poses_dict_list = []
                    for stage_idx, (start, end) in enumerate(start_end):
                        ...
                        for idx, (gripper_open, obj_pose, obj_pose_relative_to_target, target_pose) in enumerate(zip(
                            ...
                            [obs.misc[f"stage{stage_idx}_grasp_obj_pose"] for obs in demo],              # !! use stage object pose
                            [obs.misc[f"stage{stage_idx}_obj_pose_relative_to_target"] for obs in demo],
                            [obs.misc[f"stage{stage_idx}_target_obj_pose"] for obs in demo])
                            ):

                            # only consider poses in current stage
                            if (not idx >= start) or (not idx <= end+1):    # the end keyframe is collected right after opening
                                continue

                            # !! enforce gripper_open == 0 would result in losing end keyframe (the end keyframe is collected right after opening)
                        ...
                        if len(poses_dict["grasp_obj_pose"]) <= 0:
                            print(f"Skip episode{ex_idx} stage{stage_idx}. Gripper always open.")
                            break

                        poses_dict_list.append(poses_dict)

                    arrays_sub_dict_list, total_count_sub_list = collect_narr_function(poses_dict_list, None)
...
def main(argv):
    ...
    if FLAGS.all_variations:
        ...
        # !! multiple stages are only useful for multi-variant setting
