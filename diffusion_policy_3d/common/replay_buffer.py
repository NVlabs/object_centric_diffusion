...
class ReplayBuffer:
    ...
    def create_dummy(cls):
        dummy_state = np.stack([np.repeat(i, 7) for i in range(9)])             # (9, 7)
        dummy_state_next = np.stack([np.repeat(i, 7) for i in range(1, 10)])    # (9, 7)
        dummy_action = np.stack([np.repeat(i, 7) for i in range(11, 20)])       # (9, 7)
        # print(dummy_state.shape, dummy_state_next.shape, dummy_action.shape)
        root = {
            'data': {
                'state': dummy_state,
                'state_next': dummy_state_next,
                'action': dummy_action,
            },
            'meta': {
                'episode_ends': np.array([9], dtype=np.int64)
            }
        }
        return cls(root=root)
