...
class ConditionalUnet1D_progress(nn.Module):
    def __init__(self, 
        input_dim,
        local_cond_dim=None,
        ...
        actoin_dim = input_dim
        final_conv = nn.Sequential(
            Conv1dBlock(start_dim, start_dim, kernel_size=kernel_size),
            nn.Conv1d(start_dim, actoin_dim, 1),
        )
        progress_dim = 1
        # final_conv2 = nn.Sequential(
        #     Conv1dBlock(start_dim, start_dim, kernel_size=kernel_size),
        #     nn.Conv1d(start_dim, progress_dim, 1),
        #     nn.Sigmoid(),
        # )
        final_conv2 = nn.Sequential(
        ...
        # final_conv2 = nn.Sequential(
        #     nn.Conv1d(input_dim, start_dim, 1),
        #     nn.Conv1d(start_dim, progress_dim, 1),
        #     nn.Sigmoid(),
        # )
    ...
    def forward(self, 
        ...
        x_action = self.final_conv(x)
        x_action = einops.rearrange(x_action, 'b t h -> b h t')

        x_progress = self.final_conv2(x2)
        x_progress = einops.rearrange(x_progress, 'b t h -> b h t')

        x = torch.concat([x_action, x_progress], dim=-1)
