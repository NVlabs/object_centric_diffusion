...
class TrainDP3Workspace:
    ...
    def eval(self):
        ...
        fp_debug_dir = "data/debug/fp"
        pose_estimation_wrapper = FoundationPoseWrapper(
            mesh_dir=cfg.pose_estimation.mesh_dir,
            debug_dir=fp_debug_dir,
        )
        ...
        eval_epoch = cfg.evaluation.eval_epoch
        tag = f'epoch={eval_epoch:04d}'
        ...
        policy.eval()
        policy.cuda()
        runner_log = env_runner.run(policy, tag=tag, save_video=True)
        ...
        # save result
        if env_runner.use_fp:
            save_path = os.path.join(self.output_dir, f"eval_use_fp=true_{cfg.task.task_name}")
        else:
            save_path = os.path.join(self.output_dir, f"eval_use_fp=false_{cfg.task.task_name}")
        n_exp = len(runner_log["success"])
        if n_exp >= 25:
            json_path = os.path.join(save_path, f'{tag}.json')
            from utils.io_utils import save_np_dict_to_json
            save_np_dict_to_json(runner_log, json_path)
        else:
            print(f"Incomplete runs ({n_exp}/{25}). No log output.")
